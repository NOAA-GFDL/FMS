          ...
          init: IF ( npass == 1 ) THEN ! Checking for files only
             IF ( is_a_file(TRIM(record_line)) ) THEN
                ...
                ELSE IF ( mystat < 0 ) THEN
                   ...
                   CYCLE parser
                ELSE IF ( (diag_subset_output == DIAG_OTHER .AND. INDEX('ocean', lowercase(temp_file%file_name)) .NE. 0).OR.&
                     &    (diag_subset_output == DIAG_OCEAN .AND. INDEX('ocean', lowercase(temp_file%file_name)) .EQ. 0) ) THEN
                   CYCLE parser
             ...
             IF ( .NOT.is_a_file(TRIM(record_line)) ) THEN
                ...
                ELSE IF ( mystat < 0 ) THEN
                   IF ( mpp_pe() == mpp_root_pe() )&
                        ...
                        & TRIM(local_err_msg)//" (line: "//TRIM(line_number)//").",WARNING)
                   CYCLE parser
                ELSE IF ( (diag_subset_output == DIAG_OTHER .AND. INDEX('ocean', lowercase(temp_field%file_name)) .NE. 0).OR.&
                     &    (diag_subset_output == DIAG_OCEAN .AND. INDEX('ocean', lowercase(temp_field%file_name)) .EQ. 0) ) THEN
                   CYCLE parser
