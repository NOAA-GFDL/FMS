...
use fms_diag_util_mod, only: compare_two_sets_of_bounds, real_copy_set, check_indices_order, init_mask_3d
#endif
#if defined(_OPENMP)
use omp_lib
#endif
...
function fms_diag_do_reduction(this, field_data, diag_field_id, oor_mask, weight, &
  ...
  integer :: l_start(3) !< local start indices on 3 axes for regional output
  integer :: l_end(3) !< local end indices on 3 axes for regional output
  ...
  !! The following subroutine has not been merged yet.
  ...
  do i = 1, size(this%FMS_diag_fields(diag_field_id)%buffer_ids)
    ...
    !!TODO: If sub regional output, get starting and ending indices
    if (is_regional) then
      !!TODO: Save locally the indices
    end if
    ...
    case (time_max)
      call fms_diag_update_extremum(1, ptr_diag_buffer_obj, field_data, updated_bounds, l_start, &
        l_end, is_regional, reduced_k_range, sample, oor_mask, field_name, has_diurnal_axis, err_msg=err_msg)
    case (time_min)
      call fms_diag_update_extremum(0, ptr_diag_buffer_obj, field_data, updated_bounds, l_start, &
        l_end, is_regional, reduced_k_range, sample, oor_mask, field_name, has_diurnal_axis, err_msg=err_msg)
    case (time_sum)
