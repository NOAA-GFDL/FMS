...
!> @{
  ...
  subroutine AXIS_UTILS_TYPE_EQ_(axis_utils_out, axis_utils_in)
      type(axis_utils_type), intent(inout) :: axis_utils_out
      type(axis_utils_type), intent(in)    :: axis_utils_in

      axis_utils_out%



  end subroutine AXIS_UTILS_TYPE_EQ_ 
  ...
  real(kind=FMS_AU_KIND_), dimension(:),   allocatable :: r32
  real(kind=FMS_AU_KIND_), dimension(:,:), allocatable :: r322d
  real(kind=FMS_AU_KIND_), dimension(:),   allocatable :: r64
  real(kind=FMS_AU_KIND_), dimension(:,:), allocatable :: r642d
  integer :: i
  integer :: n
  ...
  if (trim(buffer) .ne. "") then
    ...
    elseif (size(dim_sizes) .eq. 2) then
      ...
      select type (edge_data)
        type is (real(kind=FMS_AU_KIND_))
    ...
    select type (edge_data)
      type is (real(kind=FMS_AU_KIND_))
        ...
        do i = 2, n
          edge_data(i) = r32(i-1) + 0.5_FMS_AU_KIND_*(r32(i) - r32(i-1))
        enddo
        edge_data(1) = r32(1) - 0.5_FMS_AU_KIND_*(r32(2) - r32(1))
        if (abs(edge_data(1)) .lt. real(1.e-10,kind=FMS_AU_KIND_)) then
          edge_data(1) = 0._FMS_AU_KIND
        endif
        edge_data(n+1) = r32(n) + 0.5_FMS_AU_KIND_*(r32(n) - r32(n-1))
        deallocate(r32)
      type is (real(kind=FMS_AU_KIND_))
        allocate(r64(n))
        call read_data(fileobj, name, r64)
        do i = 2, n
          edge_data(i) = r64(i-1) + 0.5_FMS_AU_KIND_*(r64(i) - r64(i-1))
        enddo
        edge_data(1) = r64(1) - 0.5_FMS_AU_KIND_*(r64(2) - r64(1))
        if (abs(edge_data(1)) .lt. real(1.d-10, kind=FMS_AU_KIND_)) then
          edge_data(1) = 0._FMS_AU_KIND_
        endif
        edge_data(n+1) = r64(n) + 0.5_FMS_AU_KIND_*(r64(n) - r64(n-1))
        deallocate(r64)
      class default
        call mpp_error(FATAL, "axis_edges: unsupported kind.")
    end select
  ...
  function LON_IN_RANGE_(lon, l_strt)
    ...
    real(kind=FMS_AU_KIND_)             :: lon_in_range
    real(kind=FMS_AU_KIND_)             :: l_end

    lon_in_range = lon
    l_end = l_strt+360.0_FMS_AU_KIND_

    if (abs(lon_in_range - l_strt) < real(1.e-4,kind=FMS_AU_KIND_)) then
      lon_in_range = l_strt
      return
    endif

    if (abs(lon_in_range - l_end) < real(1.e-4,kind=FMS_AU_KIND_)) then
      lon_in_range = l_strt
      return
    endif
  ...
  function FRAC_INDEX_(value, array)
    ...
    real(kind=FMS_AU_KIND_) :: frac_index
 ...
 !       if (value > array(ia)) frac_index = float(ia)
  ...
  subroutine INTERP_1D_CUBIC_SPLINE_(grid1, grid2, data1, data2, yp1, ypn)
    ...
    if (yp1>real(.99e30,kind=FMS_AU_KIND_)) then
       y2(1)=0._FMS_AU_KIND_
       u(1)=0._FMS_AU_KIND_
    else
       y2(1)=-0.5_FMS_AU_KIND_
       u(1)=(3._FMS_AU_KIND_)/(grid1(2)-grid1(1))*((data1(2)-data1(1))/(grid1(2)-grid1(1))-yp1)
    endif

    do i=2,n-1
       sig=(grid1(i)-grid1(i-1))/(grid1(i+1)-grid1(i-1))
       p=sig*y2(i-1) + 2._FMS_AU_KIND_
       y2(i)=(sig-1.)/p
       u(i)=(6._FMS_AU_KIND_*((data1(i+1)-data1(i))/(grid1(i+1)-grid1(i))-(data1(i)-data1(i-1)) &
    ...
    if (ypn>real(.99e30,kind=FMS_AU_KIND_)) then
       ...
       qn=0.5_FMS_AU_KIND_
       un=(3._FMS_AU_KIND_)/(grid1(n)-grid1(n-1))*(ypn-(data1(n)-data1(n-1))/(grid1(n)-grid1(n-1)))
    endif

    y2(n)=(un-qn*u(n-1))/(qn*y2(n-1)+1._FMS_AU_KIND_)
    ...
    do k = 1, m
       ...
       data2(k) = a*data1(klo) + b*data1(khi)+ ((a**3-a)*y2(klo) + (b**3-b)*y2(khi))*(h**2)/6._FMS_AU_KIND_
    enddo
 ...
 !###################################################################
  ...
  subroutine INTERP_1D_3D_(grid1,grid2,data1,data2, method, yp1, yp2)
    ...
    class(*), dimension(:,:,:),           intent(in)    :: grid1, data1, grid2
    class(*), dimension(:,:,:),           intent(inout) :: data2
    character(len=*),           optional, intent(in)    :: method
    real(kind=FMS_AU_KIND_),    optional, intent(in)    :: yp1, yp2
