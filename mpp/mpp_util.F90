...
#include <fms_platform.h>
  ...
  use mpp_parameter_mod, only : FATAL, WARNING, NOTE, MAX_BINS, PESET_MAX, CLOCK_LOOP  
  use mpp_parameter_mod, only : MAX_CLOCKS, MAX_EVENT_TYPES, EVENT_ALLREDUCE, EVENT_BROADCAST
  use mpp_parameter_mod, only : EVENT_RECV, EVENT_SEND, EVENT_WAIT, MAX_EVENTS, MPP_WAIT, MAXPES
  ...
  use mpp_data_mod,      only : previous_clock, num_clock_ids, mpp_record_timing_data
  use mpp_data_mod,      only : first_call_system_clock_mpi, mpi_tick_rate, mpi_count0
  use mpp_data_mod,      only : ticks_per_sec, max_ticks, start_tick, end_tick, stat, request
  use mpp_data_mod,      only : status, ptr_status, sync, ptr_sync, in_unit, out_unit, err_unit
...
#include <mpif.h>   !sgi_mipspro gets this from 'use mpi'
  ...
  !--- public interface
  public :: stdin, stdout, stderr, stdlog, mpp_error, mpp_pe, mpp_root_pe, mpp_npes
  public :: mpp_node, mpp_set_root_pe, mpp_declare_pelist,  mpp_set_current_pelist
