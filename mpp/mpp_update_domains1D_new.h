...
!put forward and backward halos
      ...
      do i = 0,size(domain%list)-1
         mustput = .FALSE.
         stkpos0 = stkpos
         if( BTEST(flags,0) .AND. domain%list(i)%mustputf )then
             mustput = .TRUE.
             nwords = domain%list(i)%putf%size * size(field,2)
             if( debug )then
                 write(stderr,'(a,i3,x,2i4,x,3i4,i5)')'->W: pe, (beg,end), is,ie,k,   to=', pe, stkpos+1, stkpos+nwords, &
                      domain%list(i)%putf%begin, domain%list(i)%putf%end, size(field,2), domain%list(i)%pe
             end if
...
!is reshape better than pack? 
             work(stkpos+1:stkpos+nwords) = RESHAPE( field(domain%list(i)%putf%begin:domain%list(i)%putf%end,:), (/nwords/) )
             stkpos = stkpos+nwords
         end if
         if( BTEST(flags,1) .AND. domain%list(i)%mustputb )then
             mustput = .TRUE.
             nwords = domain%list(i)%putb%size * size(field,2)
             if( debug )then
                 write(stderr,'(a,i3,x,2i4,x,3i4,i5)')'->E: pe, (beg,end), is,ie,k,   to=', pe, stkpos+1, stkpos+nwords, &
                      domain%list(i)%putb%begin, domain%list(i)%putb%end, size(field,2), domain%list(i)%pe
             end if
             if( (stkpos+nwords)*words_per_long.GT.mpp_domains_stack_size )then
                 write( text, '(i8)' )stkpos+nwords
                 call mpp_error( FATAL, &
...
!is reshape better than pack? 
             work(stkpos+1:stkpos+nwords) = RESHAPE( field(domain%list(i)%putb%begin:domain%list(i)%putb%end,:), (/nwords/) )
             stkpos = stkpos+nwords
         end if
         if( mustput )call mpp_send( work(stkpos0+1:stkpos), stkpos-stkpos0, domain%list(i)%pe )
...
!get forward and backward halos
      do i = 0,size(domain%list)-1
         stkpos0 = stkpos
         mustget = .FALSE.
         if( BTEST(flags,0) .AND. domain%list(i)%mustgetb )then
             mustget = .TRUE.
             nwords = domain%list(i)%getb%size * size(field,2)
             if( debug )then
                 write(stderr,'(a,i3,x,2i4,x,3i4,i5)')'<-W: pe, (beg,end), is,ie,k, from=', pe, stkpos+1, stkpos+nwords, &
                      domain%list(i)%getb%begin, domain%list(i)%getb%end, size(field,2), domain%list(i)%pe
             end if
             if( (stkpos+nwords)*words_per_long.GT.mpp_domains_stack_size )then
                 write( text, '(i8)' )stkpos+nwords
                 call mpp_error( FATAL, &
         ...
         if( BTEST(flags,1) .AND. domain%list(i)%mustgetf )then
             mustget = .TRUE.
             nwords = domain%list(i)%getf%size * size(field,2)
             if( debug )then
                 write(stderr,'(a,i3,x,2i4,x,3i4,i5)')'<-E: pe, (beg,end), is,ie,js,je,k, from=', pe, stkpos+1, stkpos+nwords, &
                      domain%list(i)%getf%begin, domain%list(i)%getf%end, size(field,2), domain%list(i)%pe
             end if
             if( (stkpos+nwords)*words_per_long.GT.mpp_domains_stack_size )then
                 write( text, '(i8)' )stkpos+nwords
                 call mpp_error( FATAL, &
         ...
         if( mustget )then
             call mpp_recv( work(stkpos0+1:stkpos), stkpos-stkpos0, domain%list(i)%pe )
             if( debug )write( stderr, '(a,i4/(8f9.6))' )'pe, work=', pe, work(stkpos0+1:stkpos)
             stkpos = stkpos0
             if( BTEST(flags,0) .AND. domain%list(i)%mustgetb )then
!is reshape better than unpack? 
                 field(domain%list(i)%getb%begin:domain%list(i)%getb%end,:) = &
                      RESHAPE( work(stkpos+1:stkpos+nwords), (/domain%list(i)%getb%size,size(field,2)/) )
                 if( debug )write( stderr, '(a,3i4/(8f9.6))' ) 'getb: pe, is, ie=', &
                        pe, domain%list(i)%getb%begin,domain%list(i)%getb%end, &
                      field(domain%list(i)%getb%begin:domain%list(i)%getb%end,:) 
                 stkpos = stkpos+nwords
                 call mpp_set_active_domain( domain, begin=domain%list(i)%getb%begin )
             end if
             if( BTEST(flags,1) .AND. domain%list(i)%mustgetf )then
!is reshape better than unpack? 
                 field(domain%list(i)%getf%begin:domain%list(i)%getf%end,:) = &
                      RESHAPE( work(stkpos+1:stkpos+nwords), (/domain%list(i)%getf%size,size(field,2)/) )
                 if( debug )write( stderr, '(a,3i4/(8f9.6))' ) 'getf: pe, is, ie=', &
                        pe, domain%list(i)%getf%begin,domain%list(i)%getf%end, &
                      field(domain%list(i)%getf%begin:domain%list(i)%getf%end,:) 
                 stkpos = stkpos+nwords
                 call mpp_set_active_domain( domain, end=domain%list(i)%getf%end )
             end if
         end if
      end do
      call mpp_sync_self()
