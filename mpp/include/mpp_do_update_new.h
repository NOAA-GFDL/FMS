! -*-f90-*- 
    subroutine MPP_DO_UPDATE_3Dnew_( f_addrs, domain, d_type, ke, b_addrs, b_size, flags, name)
         ...
         do list = 0,nlist-1
            ...
            if( msgsize.GT.0 )then
               ...
               if( mpp_domains_stack_hwm.GT.mpp_domains_stack_size )then
                  ...
                  call mpp_error( FATAL, 'MPP_DO_UPDATE_NEW: mpp_domains_stack overflow, ' // &
                     ...
                     do j = js, je
                        do i = is, ie
                           ...
                           if( field(i,j,k) .NE. buffer(pos) ) then
                              print*,"Error from MPP_DO_UPDATE_NEW on pe = ", mpp_pe(), ": field ", &
                                   trim(field_name), " at point (", i, ",", j, ",", k, ") = ", field(i,j,k), &
                                   " does not equal to the value = ", buffer(pos), " on pe ", domain%list(m)%pe
                              call mpp_error(debug_update_level, "MPP_DO_UPDATE_NEW: mismatch on the boundary for symmetry point")
                              exit CHECK_LOOP
                           end if
                        end do
                     end do
...
!     ptr_rfield = f_addrs(1)
                     ...
                     if(OverPtr%is_refined(n)) then
                        ...
                        total = ni*nj
                              ...
                              if(start+total*ke>b_size ) call mpp_error(FATAL, &
                                   "MPP_DO_UPDATE_NEW: b_size is less than the size of the data to be filled.")
                        msgsize = ie - is + 1
                              do l=1, l_size  ! loop over number of fields
      ...
      return
    end subroutine MPP_DO_UPDATE_3Dnew_
