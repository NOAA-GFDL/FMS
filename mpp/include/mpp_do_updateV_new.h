! -*-f90-*- 
    subroutine MPP_DOMAINS_DO_UPDATE_3Dnew_V_(f_addrsx,f_addrsy, domainx, domainy, d_type, ke, flags, gridtype, name)
!updates data domain of 3D field whose computational domains have been computed
      ...
      integer :: update_flags, grid_offset_type, sign
      integer :: l_size, l, i, j, k, is, ie, js, je, m, n, is2, ie2, js2, je2
      integer :: pos, nlist, list, msgsize, nsend, nrecv
      integer :: to_pe, from_pe, midpoint, ioff, joff
      integer :: ntileMe, ntileNbr, ndir, tMe, tNbr, dir
                     ...
                     do tNbr = 1, ntileNbr 
                        ...
                        do n = 1, 3
                           if( overPtrx%overlap(n) )then
                              is = overPtrx%is(n); ie = overPtrx%ie(n)
                              js = overPtrx%js(n); je = overPtrx%je(n)
                              if( overPtrx%rotation == ZERO .OR. overPtrx%rotation == ONE_HUNDRED_EIGHTY ) then
                                 do k = 1,ke
                                    do j = js, je
                                       do i = is, ie
                                          pos = pos + 1
                           ...
                           if( overPtry%overlap(n) )then
                              ...
                              if( overPtry%rotation == ZERO .OR. overPtry%rotation == ONE_HUNDRED_EIGHTY ) then
                                 do k = 1,ke
                                    do j = js, je
                                       do i = is, ie
                                          pos = pos + 1
...
!        write(stdout(),*) 'Update field checksum=',mpp_chksum(rfield)
     ...
     ! ---northern boundary fold
            ...
            if( je.GE.js )then
               ...
               case (CGRID_NE)
                  if(domainx%symmetry) then
                     ...
                     js2 = domainy%y(1)%global%end + joff + 1; je2 = domainy%y(1)%data%end + joff                     
                     do l=1,l_size
                        ptr_fieldx = f_addrsx(l)
                        ptr_fieldy = f_addrsy(l)   
                        do k = 1,ke
      ...
      if( debug .AND. domainx%symmetry .AND.  grid_offset_type .NE. AGRID ) then      
         ...
         ndir = size(bound_x%list(0)%send,3)
         ...
         do list = 0,nlist-1
            ...
            do dir = 1, ndir  ! ndir = 1 for E or N-cell, ndir = 2 for C-cell
               do l = 1, l_size ! loop over number of fields
                  ...
                  ptr_fieldy = f_addrsy(l)
                  do tNbr = 1, ntileNbr
                     overPtrx => bound_x%list(m)%send(tNbr, tMe, dir)
                     overPtry => bound_y%list(m)%send(tNbr, tMe, dir)
         ...
         do list = 0,nlist-1
            ...
            do dir = 1, ndir  
               do tNbr = 1, ntileNbr
                  do l = 1, l_size  ! loop over number of fields
                     tMe = min(l, ntileMe)
                     overPtrx => bound_x%list(m)%recv(tMe, tNbr, dir)
                     overPtry => bound_y%list(m)%recv(tMe, tNbr, dir)
                     if( overPtrx%overlap(1) ) then
                        is = overPtrx%is(1); ie = overPtrx%ie(1)
                        js = overPtrx%js(1); je = overPtrx%je(1)
                        msgsize = msgsize + (ie-is+1)*(je-js+1)
         ...
         do list = nlist-1,0,-1
            ...
            ntileNbr = size(domainx%list(m)%x(:))
            pos = buffer_pos
            do dir = ndir, 1, -1  ! ndir = 1 for E or N-cell, ndir = 2 for C-cell
               do tNbr = ntileNbr, 1, -1
                  do l=l_size,1,-1  ! loop over number of fields
                     tMe = min(l, ntileMe)
                     ptr_fieldx = f_addrsx(l)
                              ...
                              do j = js, je
                                 do i = is, ie
                                    ...
                                    if( fieldy(i,j,k) .NE. buffer(pos) ) then
                                       print*,"Error from mpp_do_updateV_new.h on pe = ", mpp_pe(), ": y component of vector ", &
                                            trim(field_name), " at point (", i, ",", j, ",", k, ") = ", fieldy(i,j,k), &
                                            " does not equal to the value = ", buffer(pos), " on pe ", domainx%list(m)%pe
                                       call mpp_error(FATAL, "mpp_do_updateV_new.h: mismatch on the boundary for symmetry point")
                                    end if
                                 end do
                              end do
                           end do
                              ...
                              do j = js, je
                                 do i = is, ie
                                    ...
                                    if( fieldx(i,j,k) .NE. buffer(pos) ) then
                                       print*,"Error from mpp_do_updateV_new.h on pe = ", mpp_pe(), ": x-component of vector ", &
            ...
            end do ! end do dir = ndir, 1, -1
         end do ! end do list = nlist-1,0,-1
         write(stdout(),*) "NOTE from mpp_do_updateV_new.h: the data on the boundary is consistent for field " &
                           //trim(field_name)
      end if
