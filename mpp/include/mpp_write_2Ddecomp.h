    ...
    subroutine MPP_WRITE_2DDECOMP_3D_( unit, field, domain, data, tstamp, tile_number)
...
!NEW: data may be on compute OR data domain
      logical :: data_has_halos, halos_are_global, x_is_global, y_is_global, shift_exist
      ...
      call mpp_get_compute_domain( domain, is,  ie,  js,  je, tile_number=tile_number  )
      call mpp_get_data_domain   ( domain, isd, ied, jsd, jed, x_is_global=x_is_global, &
                                   y_is_global=y_is_global, tile_number=tile_number )
      call mpp_get_memory_domain ( domain, ism, iem, jsm, jem )
      call mpp_get_global_domain ( domain, isg, ieg, jsg, jeg, tile_number=tile_number )
      position = CENTER
      ishift = 0; jshift = 0
      shift_exist = .false.
...
!store compute domain as contiguous data and pass to write_record
          ...
          call write_record( unit, field, size(cdata(:,:,:)), cdata, tstamp, domain, tile_number=tile_number ) 
      else if( shift_exist)then
!store compute domain as contiguous data and pass to write_record
          allocate( cdata(is:ie,js:je,size(data,3)) )
          cdata(:,:,:) = data(1:ie-is+1,1:je-js+1,:)
          call write_record( unit, field, size(cdata(:,:,:)), cdata, tstamp, domain, tile_number=tile_number )

      else
!data is already contiguous
          call write_record( unit, field, size(data(:,:,:)), data, tstamp, domain, tile_number=tile_number )
      end if
