! -*-f90-*- 
    subroutine MPP_DO_UPDATE_3D_V_(f_addrsx,f_addrsy, domainx, domainy, d_type, ke, b_addrsx, b_addrsy, &
                     ...
                     do j = js, je
                        do i = is, ie
                           ...
                           if( fieldy(i,j,k) .NE. buffer(pos) ) then
                              print*,"Error from MPP_DO_UPDATE_V on pe = ", mpp_pe(), ": y component of vector ", &
                                   trim(field_name), " at point (", i, ",", j, ",", k, ") = ", fieldy(i,j,k), &
                                   " does not equal to the value = ", buffer(pos), " on pe ", domainx%list(m)%pe
                              call mpp_error(debug_update_level, "MPP_DO_UPDATE_V: mismatch on the boundary for symmetry point")
                              exit CHECK_LOOP
                           end if
                        end do
                     end do
                     ...
                     do j = js, je
                        do i = is, ie
                           ...
                           if( fieldx(i,j,k) .NE. buffer(pos) ) then
                              print*,"Error from MPP_DO_UPDATE_V on pe = ", mpp_pe(), ": x-component of vector ", &
         ...
         case(BGRID_NE, BGRID_SW, AGRID)
            do n = 1, overPtrx%count
               ...
               if( send(dir) ) then
                  ...
                  end if ! if( overPtrx%is_refined(n) )
               end if ! if( send(dir) ) 
