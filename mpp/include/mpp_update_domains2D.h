    subroutine MPP_UPDATE_DOMAINS_2D_( field, domain, flags, complete, free, list_size, dc_handle )
...
!updates data domain of 3D field whose computational domains have been computed
      ...
      if(PRESENT(complete) .or. PRESENT(free))then
        ...
        if(free_comm)then
          ...
          if(l_size == 1)then
            isize=size(field,1); jsize=size(field,2); ke = size(field,3)
          else
            set_mismatch = .false.
          ...
          if(do_update)then
            ...
            if(.not.ASSOCIATED(d_comm))then  ! d_comm needs initialization or lookup
              d_comm =>mpp_update_init_comm(domain,f_addrs(1:l_size),isize,jsize,ke,flags=flags)
    ...
    subroutine MPP_UPDATE_DOMAINS_4D_( field, domain, flags, complete, free, list_size, dc_handle )
!updates data domain of 4D field whose computational domains have been computed
    ...
    subroutine MPP_UPDATE_DOMAINS_5D_( field, domain, flags, complete, free, list_size, dc_handle )
!updates data domain of 5D field whose computational domains have been computed
    ...
    subroutine MPP_REDISTRIBUTE_2D_( domain_in, field_in, domain_out, field_out, complete, free, list_size, dc_handle )
    ...
    subroutine MPP_REDISTRIBUTE_3D_( domain_in, field_in, domain_out, field_out, complete, free, list_size, dc_handle )
      type(domain2D), intent(in) :: domain_in, domain_out
      MPP_TYPE_, intent(in)  :: field_in (:,:,:)
      MPP_TYPE_, intent(out) :: field_out(:,:,:)
      logical, intent(in), optional :: complete, free
    ...
    subroutine MPP_REDISTRIBUTE_4D_( domain_in, field_in, domain_out, field_out, complete, free, list_size, dc_handle )
      type(domain2D), intent(in) :: domain_in, domain_out
      MPP_TYPE_, intent(in)  :: field_in (:,:,:,:)
      MPP_TYPE_, intent(out) :: field_out(:,:,:,:)
      logical, intent(in), optional :: complete, free
    ...
    subroutine MPP_REDISTRIBUTE_5D_( domain_in, field_in, domain_out, field_out, complete, free, list_size, dc_handle )
      type(domain2D), intent(in) :: domain_in, domain_out
      MPP_TYPE_, intent(in)  :: field_in (:,:,:,:,:)
      MPP_TYPE_, intent(out) :: field_out(:,:,:,:,:)
      logical, intent(in), optional :: complete, free
...
!updates data domain of 3D field whose computational domains have been computed
      ...
      ke = size(fieldx,3)
          ...
          if(l_size == 1)then
            isize=size(fieldx,1); jsize=size(fieldx,2); ke = size(fieldx,3)
                  else
            set_mismatch = .false.
            set_mismatch = set_mismatch .OR. (isize /= size(fieldx,1))
            set_mismatch = set_mismatch .OR. (jsize /= size(fieldx,2))
            set_mismatch = set_mismatch .OR. (ke /= size(fieldx,3))
            if(set_mismatch)then
              write( text,'(i2)' ) l_size
              call mpp_error(FATAL,'MPP_UPDATE_3D: Incompatible field at count '//text//' for group vector update.' )
                  end if
              end if
          if(do_update)then
            call mpp_do_update(f_addrsx(1:l_size),f_addrsy(1:l_size),isize,jsize,ke,d_type,domain,flags,gridtype,dc_handle)
