! -*-f90-*-
    subroutine MPP_UPDATE_DOMAINS_AD_2D_( field, domain, flags, complete, free, list_size, &
                                       position, whalo, ehalo, shalo, nhalo, name, tile_count)
!updates data domain of 2D field whose computational domains have been computed
      ...
      pointer( ptr, field3D )
      ptr = LOC(field)
      call mpp_update_domains_ad( field3D, domain, flags, complete, free, list_size, &
                               position, whalo, ehalo, shalo, nhalo, name, tile_count)
#else
      field3D = RESHAPE( field, SHAPE(field3D) )
      call mpp_update_domains_ad( field3D, domain, flags, position=position, whalo=whalo, ehalo=eahlo, &
    ...
    subroutine MPP_UPDATE_DOMAINS_AD_3D_( field, domain, flags, complete, free, list_size, &
                                       position, whalo, ehalo, shalo, nhalo, name, tile_count)
!updates data domain of 3D field whose computational domains have been computed
      ...
      if(free_comm)then
         ...
         if( domain_update_is_needed(domain, update_whalo, update_ehalo, update_shalo, update_nhalo) )then
            ...
            call mpp_do_update_ad( field, Dom, flags, name)
         end if
      endif
#else
      if(ntile>1) call mpp_error(FATAL,'MPP_UPDATE_AD_3D: when use_CRI_pointers is not defined, '// &
    ...
    subroutine MPP_UPDATE_DOMAINS_AD_4D_( field, domain, flags, complete, free, list_size, &
                                       position, whalo, ehalo, shalo, nhalo, name, tile_count)
!updates data domain of 4D field whose computational domains have been computed
    ...
    subroutine MPP_UPDATE_DOMAINS_AD_5D_( field, domain, flags, complete, free, list_size, &
                                       position, whalo, ehalo, shalo, nhalo, name, tile_count )
...
!vector fields
    subroutine MPP_UPDATE_DOMAINS_AD_2D_V_( fieldx, fieldy, domain, flags, gridtype, complete, free, &
                                         list_size, whalo, ehalo, shalo, nhalo, name, tile_count)
!updates data domain of 2D field whose computational domains have been computed
      ...
      ptrx = LOC(fieldx)
      ptry = LOC(fieldy)
      call mpp_update_domains_ad( field3Dx, field3Dy, domain, flags, gridtype, complete, free, &
                               list_size, whalo, ehalo, shalo, nhalo, name, tile_count )
#else
      field3Dx = RESHAPE( fieldx, SHAPE(field3Dx) )
      field3Dy = RESHAPE( fieldy, SHAPE(field3Dy) )
      call mpp_update_domains_ad( field3Dx, field3Dy, domain, flags, gridtype, whalo=whalo, ehalo=eahlo, &
                               shalo=shalo, nhalo=nahlo, name=name )
    ...
    subroutine MPP_UPDATE_DOMAINS_AD_3D_V_( fieldx, fieldy, domain, flags, gridtype, complete, free, &
                                         list_size, whalo, ehalo, shalo, nhalo, name, tile_count)
!updates data domain of 3D field whose computational domains have been computed
      MPP_TYPE_,        intent(inout)        :: fieldx(:,:,:), fieldy(:,:,:)
      type(domain2D),   intent(inout)        :: domain
      ...
      if(free_comm)then
         ...
         if( domain_update_is_needed(domain, update_whalo, update_ehalo, update_shalo, update_nhalo) )then
            ...
            domainy => search_domain(domain, update_whalo, update_ehalo, update_shalo, update_nhalo, &
                 gridtype=grid_offset_type, direction='y')
            if(exchange_uv) then
               call mpp_do_update_ad( fieldx, fieldy, domainy, domainx, grid_offset_type, flags, name )
            else
               call mpp_do_update_ad( fieldx, fieldy, domainx, domainy, grid_offset_type, flags, name )
            end if
         end if
      end if
#else 
      if(ntile>1) call mpp_error(FATAL,'MPP_UPDATE_AD_3D_V: when use_CRI_pointers is not defined, '// &
                  'number of tiles on each pe should be 1')
      if( domain_update_is_needed(domain, update_whalo, update_ehalo, update_shalo, update_nhalo) )then 
         domainx => search_domain(domain, update_whalo, update_ehalo, update_shalo, update_nhalo, &
              gridtype=grid_offset_type, direction='x')
    ...
    subroutine MPP_UPDATE_DOMAINS_AD_4D_V_( fieldx, fieldy, domain, flags, gridtype, complete, free, &
                                         list_size, whalo, ehalo, shalo, nhalo, name, tile_count )
!updates data domain of 4D field whose computational domains have been computed
      MPP_TYPE_,        intent(inout)        :: fieldx(:,:,:,:), fieldy(:,:,:,:)
      type(domain2D),   intent(inout)        :: domain
    ...
    subroutine MPP_UPDATE_DOMAINS_AD_5D_V_( fieldx, fieldy, domain, flags, gridtype, complete, free, &
                                         list_size, whalo, ehalo, shalo, nhalo, name, tile_count )
!updates data domain of 5D field whose computational domains have been computed
      MPP_TYPE_,        intent(inout)        :: fieldx(:,:,:,:,:), fieldy(:,:,:,:,:)
      type(domain2D),   intent(inout)        :: domain
