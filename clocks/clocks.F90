...
!clocks are stored in this internal type
  ...
  character(len=256), private :: version='$Id: clocks.F90,v 2.2 2001/02/14 19:06:12 vb Exp $'
...
!initialize clocks and reference tick
      ...
      start_tick = ref_tick
      if( verbose )then
          print '(a)', 'CLOCKS module '//trim(version)
          print *, 'Realtime clock resolution=', tick_rate, '(', ticks_per_sec, ' ticks/sec)'
      end if
...
!cumul_time is total measured time between clocks_init and clocks_exit
      real :: total_time, time_per_call, cumul_time
      ...
      do i = 1, max_clocks
         if( clocks(i)%calls.NE.0 )then
             ...
             print '(32x,a)', '   calls        t_call       t_total t_frac'
             print '(a32,i8,2f14.6,f7.3)', &
                  'CLOCKS: '//clocks(i)%name, &
                  clocks(i)%calls, time_per_call, total_time, total_time/cumul_time
         end if
      end do
      print '(a,f14.6)', 'CLOCKS: Total measured time: ', cumul_time
