  ...
  subroutine horiz_interp_conserve_del ( Interp )
    ...
    case (1)
      if( allocated( Interp%kind8_reals)) then
        if(associated(Interp%kind8_reals%area_src)) deallocate(Interp%kind8_reals%area_src)
        if(associated(Interp%kind8_reals%area_dst)) deallocate(Interp%kind8_reals%area_dst)
        if(associated(Interp%kind8_reals%facj))     deallocate(Interp%kind8_reals%facj)
        ...
        if(associated(Interp%kind4_reals%facj))     deallocate(Interp%kind4_reals%facj)
        if(associated(Interp%jlat))                 deallocate(Interp%jlat)
