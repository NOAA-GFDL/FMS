...
use time_manager_mod, only: time_type, operator(+), operator(-), set_time, get_time, &
                            NO_CALENDAR, THIRTY_DAY_MONTHS, NOLEAP, JULIAN, &
...
character(len=128) :: version='$Id: get_cal_time.F90,v 12.0 2005/04/14 18:01:58 fms Exp $'
...
!
! When allow_calendar_conversion=.false., then calendar must match the
...
correct_form = (trim(calendar_in_c)) == 'noleap'     .or. (trim(calendar_in_c)) == '365_day' .or. &
               ...
               (trim(calendar_in_c)) == 'no_calendar'.or. (trim(calendar_in_c)) == 'thirty_day_months'
...
if(.not.allow_calendar_conversion) then
  correct_form = (trim(calendar_in_c) == 'noleap'            .and. calendar_tm_i == NOLEAP)            .or. &
                 ...
                 (trim(calendar_in_c) == 'no_calendar'       .and. calendar_tm_i == NO_CALENDAR)
  if(.not.correct_form) then
...
if (allow_calendar_conversion) then
...
if (calendar_in_i /= calendar_tm_i) then
    if(calendar_in_i == NO_CALENDAR .or. calendar_tm_i == NO_CALENDAR) then
      call error_mesg('get_cal_time','Cannot do calendar conversion because input calendar is '// &
       valid_calendar_types(calendar_in_i)//' and time_manager is using '//valid_calendar_types(calendar_tm_i)// &
