module write_files
  ...
  ! variables for tile1
  character(5) :: tile
  real, dimension(c1_nxp,c1_nyp) :: x
  real, dimension(c1_nxp,c1_nyp) :: y
  real, dimension(c1_nx,c1_ny)   :: area
...
contains
  ...
  subroutine write_c1_tiles
    ...
    x(1,1)=0.0 ; x(2,1)=90.0 ; x(3,1)=180.0
 ...
 !-----------------------------------!
  subroutine call_fms2_io(filename, tile, x_in, y_in, area_in)
    ...
    character(*) :: filename
    character(*) :: tile
    real, dimension(c1_nxp,c1_nyp), intent(in) :: x_in, y_in
