...
use           fms_mod, only: fms_init, fms_end, file_exist, open_namelist_file, check_nml_error, close_file
use           fms_mod, only: error_mesg, field_exist, field_size
use        fms_io_mod, only: read_data, fms_io_exit
use     constants_mod, only: constants_init, pi
use  time_manager_mod, only: time_type, set_calendar_type, set_date, NOLEAP, operator(+), print_time
...
id_z  = diag_axis_init('z',  z,  'press_level', 'z', long_name='Pressure Level')
...
id_so2 = register_diag_field  ('test_data_override_mod', 'so2', (/id_x,id_y,id_z/), Time, 'SO2', 'molecules/cm2/s')
