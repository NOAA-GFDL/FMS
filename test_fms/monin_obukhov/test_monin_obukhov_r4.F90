...
program test_monin_obukhov
    ...
    real(kind=r4_kind)    :: grav, vonkarm, error, zeta_min, small, ustar_min
    real(kind=r4_kind)    :: zref, zref_t
    integer :: max_iter
  
    real(kind=r4_kind)    :: rich_crit, zeta_trans
    real(kind=r4_kind)    :: drag_min_heat, drag_min_moist, drag_min_mom
    logical :: neutral
    ...
    grav           = real(9.80, kind=r4_kind)
    vonkarm        = real(0.4,kind=r4_kind)
    error          = real(1.0e-4,kind=r4_kind)
    zeta_min       = real(1.0e-6,kind=r4_kind)
    max_iter       = 20
    small          = real(1.0e-4,kind=r4_kind)
    neutral        = .false.
    stable_option  = 1
    new_mo_option  = .false.
    rich_crit      = real(10.0,kind=r4_kind)
    zeta_trans     = real(0.5,kind=r4_kind)
    drag_min_heat  = real(1.0e-5,kind=r4_kind)
    drag_min_moist = real(1.0e-5,kind=r4_kind)
    drag_min_mom   = real(1.0e-5,kind=r4_kind)
    ustar_min      = real(1.e-10,kind=r4_kind)
  
  
    zref   = real(10.0,kind=r4_kind)
    zref_t = real(2.0,kind=r4_kind)
    ...
    CONTAINS
      ...
      subroutine test_drag
        ...
        real(kind=r4_kind), dimension(n) :: pt, pt0, z, z0, zt, zq, speed, drag_m, drag_t, drag_q, u_star, b_star
        ...
        call monin_obukhov_drag_1d(real(grav,kind=r4_kind), real(vonkarm,kind=r4_kind),                       &
        & real(error,kind=r4_kind), real(zeta_min,kind=r4_kind), max_iter, real(small,kind=r4_kind),          &               &
          neutral, stable_option, new_mo_option, real(rich_crit,kind=r4_kind),                                &!miz
        & real(zeta_trans,kind=r4_kind), real(drag_min_heat,kind=r4_kind), real(drag_min_moist,kind=r4_kind), &
        & real(drag_min_mom,kind=r4_kind), n, pt, pt0, z, z0, zt, zq, speed, drag_m, drag_t,                  &
  ...
  #define CHKSUM_DRAG 4466746452959549650
      ...
      subroutine test_stable_mix
        ...
        integer,            parameter      :: n = 5
        real(kind=r4_kind), dimension(n)   :: rich
        real(kind=r4_kind), dimension(n)   :: mix
        ...
        call monin_obukhov_stable_mix(stable_option, real(rich_crit,kind=r4_kind), &
                                      real(zeta_trans,kind=r4_kind), n, rich, mix, ier_l)
  ...
  #define CHKSUM_STABLE_MIX 4590035772608644258
      ...
      subroutine test_diff
        ...
        real(kind=r4_kind), dimension(ni, nj, nk) :: z
        real(kind=r4_kind), dimension(ni, nj)     :: u_star, b_star
        real(kind=r4_kind), dimension(ni, nj, nk) :: k_m, k_h
        ...
        call monin_obukhov_diff(real(vonkarm,kind=r4_kind),                         &
             & real(ustar_min,kind=r4_kind),                                        &
             & neutral, stable_option, new_mo_option, real(rich_crit,kind=r4_kind), &
             & real(zeta_trans,kind=r4_kind),                                       &!miz
  ...
  #define CHKSUM_DIFF -9222066590093362639
      ...
      subroutine test_profile
        ...
        real(kind=r4_kind), dimension(n) :: z, z0, zt, zq, u_star, b_star, q_star
        real(kind=r4_kind), dimension(n) :: del_m, del_t, del_q
        ...
        call monin_obukhov_profile_1d(real(vonkarm,kind=r4_kind),                   &
             & neutral, stable_option, new_mo_option, real(rich_crit,kind=r4_kind), &
             & real(zeta_trans,kind=r4_kind), n, real(zref,kind=r4_kind),           &
             & real(zref_t,kind=r4_kind), z, z0, zt, zq, u_star, b_star, q_star,    &
