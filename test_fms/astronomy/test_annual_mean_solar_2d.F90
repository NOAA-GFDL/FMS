...
program test_annual_2d
  ...
  subroutine create_ref_values_2d(jst, jnd, lat, cosz, solar, fracday, rrsun)
    ...
    real(kind=TEST_AST_KIND_), intent(in), dimension(n,n)  :: lat
    real(kind=TEST_AST_KIND_), intent(out), dimension(n,n) :: cosz, solar, fracday
    real(kind=TEST_AST_KIND_), intent(out)                 :: rrsun
    real(kind=TEST_AST_KIND_)                              :: t
    real(kind=TEST_AST_KIND_), dimension(n,n)              :: s, z
    integer                                                :: i
