...
program test_time_manager
 ...
 use time_manager2_mod, only: time_type, set_date, get_date, set_time, set_calendar_type, real_to_time_type
 use time_manager2_mod, only: length_of_year, leap_year, days_in_month, days_in_year, print_time
 use time_manager2_mod, only: set_ticks_per_second, get_ticks_per_second
 use time_manager2_mod, only: decrement_date, increment_date, get_time, increment_time, decrement_time
 use time_manager2_mod, only: JULIAN, GREGORIAN, THIRTY_DAY_MONTHS, NOLEAP
 use time_manager2_mod, only: operator(-), operator(+),  operator(*),  operator(/),  &
...
logical :: test17=.true.,test18=.true.,test19=.true.,test20=.true.,test21=.true.,test22=.true.
 ...
 outunit = open_file(file='test_time_manager_greg.out', form='formatted', action='write')
 errunit = stderr()
...
! Tests of decrement_date and increment_date
 ...
 !  This test loops through every day up to year 2200 and writes a line to the output file for each day.
