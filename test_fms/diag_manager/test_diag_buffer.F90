program test_diag_buffer
    ...
    !allocate(r8val)
    call buffobj0(5)%get_buffer(p_val)
    select type(p_val)
    ...
    allocate(arr1d(10))
    call buffobj1%get_buffer(p_data1)
    select type(p_data1)
      type is(real(4))
        print *, p_data1
        arr1d = p_data1
    ...
    !! get the buffer
    call buffobj3%get_buffer(arr3d)
    !! get the remapped buffer
    ...
    !! get the buffer
    call buffobj4%get_buffer(arr4d)
    !! get the remapped buffer
    ...
    !! get the buffer
    call buffobj5%get_buffer(arr5d)
    !! check output
    select type (arr4d)
      type is(integer(i8_kind))
        print *, arr4d
