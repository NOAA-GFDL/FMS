...
program check_time_diurnal
  ...
  do ti = 1, 8
    ...
    print *, "Checking answers for var0_diurnal - time_level:", string(ti)
    call read_data(fileobj, "var0_diurnal", cdata_out(1,1,1,1), unlim_dim_level=ti)
    call check_data_0d(cdata_out(1,1,1,1), ti)

    cdata_out = -999_r4_kind
    print *, "Checking answers for var1_diurnal - time_level:", string(ti)
    call read_data(fileobj, "var1_diurnal", cdata_out(:,1,1,1), unlim_dim_level=ti)
    call check_data_1d(cdata_out(:,1,1,1), ti)

    cdata_out = -999_r4_kind
    print *, "Checking answers for var2_diurnal - time_level:", string(ti)
    call read_data(fileobj, "var2_diurnal", cdata_out(:,:,1,1), unlim_dim_level=ti)
    call check_data_2d(cdata_out(:,:,1,1), ti)

    cdata_out = -999_r4_kind
    print *, "Checking answers for var3_diurnal - time_level:", string(ti)
    call read_data(fileobj, "var3_diurnal", cdata_out(:,:,:,1), unlim_dim_level=ti)
    call check_data_3d(cdata_out(:,:,:,1), ti, .false.)

    cdata_out = -999_r4_kind
    print *, "Checking answers for var4_diurnal - time_level:", string(ti)
    call read_data(fileobj, "var4_diurnal", cdata_out(:,:,:,:), unlim_dim_level=ti)
    call check_data_3d(cdata_out(:,:,:,1), ti, .false.)
    ...
    call read_data(fileobj1, "var3_diurnal", cdata_out(1:4,1:3,1:2,1), unlim_dim_level=ti)
    ...
    call read_data(fileobj2, "var3_diurnal", cdata_out(1:4,1:1,1:2,1), unlim_dim_level=ti)
  ...
  subroutine check_data_0d(buffer, time_level)
   ...
   !  + ( 1/100 * avg of time step increments )
    ...
    if (abs(buffer - buffer_exp) > 1.0e-4) then
      ...
      call mpp_error(FATAL, "Check_time_diurnal::check_data_0d:: Data is not correct")
    endif
