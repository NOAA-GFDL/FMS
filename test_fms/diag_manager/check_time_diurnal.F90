...
program check_time_diurnal
  ...
  do ti = 1, nmonths 
    ...
    call read_data(fileobj, "ice", cdata_out(:,:,1:nd,1), unlim_dim_level=ti)
    call check_data_2d(cdata_out(:,:,1:nd,1), ti, sample_size=3)

    cdata_out = -999_r4_kind
    print *, "Checking answers for var3_diurnal - time_level:", string(ti)
    call read_data(fileobj, "sst", cdata_out(:,:,:,:), unlim_dim_level=ti)
    call check_data_3d(cdata_out(:,:,:,:), ti, .false., sample_size=3)
...
contains
  ...
  subroutine check_data_2d(buffer, time_level, sample_size)
    ...
    integer :: d_index
    real(r8_kind) :: hrly_sums(sample_size) !< can i even do this
