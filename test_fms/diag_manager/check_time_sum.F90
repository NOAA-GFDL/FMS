...
program check_time_sum
  ...
  do i = 1, 8
    ...
    call read_data(fileobj, "var0_sum", cdata_out(1:1,1,1,1), unlim_dim_level=i) !eyeroll
    ...
    cdata_out = -999_r4_kind
    print *, "Checking answers for var3_Z_sum - time_level:", string(i)
    call read_data(fileobj, "var3_Z_sum", cdata_out(:,:,1:2,1), unlim_dim_level=i)
...
contains
  integer function basic_math(time_level)
    integer, intent(in) :: time_level

    integer :: ii

    basic_math = 0
    do ii = (time_level-1)*6+1, time_level*6
      basic_math = basic_math + ii
    enddo
  end function
  ...
  subroutine check_data_0d(buffer, time_level)
    ...
    buffer_exp = real((1000_r8_kind+10_r8_kind+1_r8_kind)* 6_r8_kind + &
      real(basic_math(time_level), kind=r8_kind)/100_r8_kind, kind=r4_kind)
  ...
  subroutine check_data_1d(buffer, time_level)
    ...
    do ii = 1, size(buffer, 1)
      buffer_exp = real((real(ii, kind=r8_kind)* 1000_r8_kind+10_r8_kind+1_r8_kind)*6_r8_kind + &
                   real(basic_math(time_level), kind=r8_kind)/100_r8_kind, kind=r4_kind)
    ...
    do ii = 1, size(buffer, 1)
      do j = 1, size(buffer, 2)
        buffer_exp = real((real(ii, kind=r8_kind)* 1000_r8_kind+ &
                     10_r8_kind*real(j, kind=r8_kind)+1_r8_kind)*6_r8_kind + &
                     real(basic_math(time_level), kind=r8_kind)/100_r8_kind, kind=r4_kind)
      ...
      do j = 1, size(buffer, 2)
        do k = 1, size(buffer, 3)
          buffer_exp = real((real(ii+nx_oset, kind=r8_kind)* 1000_r8_kind + &
                       ...
                       1_r8_kind*real(k+nz_oset, kind=r8_kind))*6_r8_kind + &
                       real(basic_math(time_level), kind=r8_kind)/100_r8_kind, kind=r4_kind)
