...
subroutine fill_coarse_data_r8(rdata, rotate, iadd, jadd, is_c, ie_c, js_c, je_c, nz, isd, jsd, nx, ny, &
    ...
    real(kind=r8_kind),    intent(inout) :: rdata(isd:, jsd:, :)
       ...
       do k = 1, nz
          do j = js_c, je_c+jshift
             do i = is_c, ie_c+ishift
                rdata(i,j,k) = dble(i+iadd)*1.d+6 + dble(j+jadd)*1.d+3 + dble(k) + x_add
             enddo
          enddo
       enddo
    case (NINETY)
       ...
       do k = 1, nz
          do j = js_c, je_c+jshift
             do i = is_c, ie_c+ishift
                rdata(i,j,k) = sign1*( dble(nx-j+1+iadd+jshift)*1.d+6 + dble(i+jadd)*1.d+3 + dble(k) + y_add)
             enddo
          enddo
       enddo
    case (MINUS_NINETY)
       ...
       do k = 1, nz
          do j = js_c, je_c+jshift
             do i = is_c, ie_c+ishift
                rdata(i,j,k) = sign2*( dble(j+iadd)*1.d+6 + dble(ny-i+1+jadd+ishift)*1.d+3 + dble(k) + y_add)
             enddo
          enddo
       enddo
    case default
       call mpp_error(FATAL,"fill_coarse_rdata: rotate_coarse must be ZERO, NINETY, MINUS_NINETY")
    end select
    ...
    if(x_cyclic) then
       if(ie_c+ishift+iadd == ieg) then
          ...
          do k = 1, nz
             do j = js_c, je_c+jshift
                rdata(i,j,k) = dble(i)*1.d+6 + dble(j+jadd)*1.d+3 + dble(k) + x_add
             enddo
          enddo
       endif
    endif
    ...
    if(y_cyclic) then
       if(je_c+jshift+jadd == jeg) then
          ...
          do k = 1, nz
             do j = js_c, je_c+jshift
                rdata(i,j,k) = dble(i+iadd)*1.d+6 + j*1.d+3 + dble(k) + x_add
             enddo
          enddo
       endif
    endif
