...
parser = get_parser()
call add_argument(parser, &
                  "-t", &
                  "Test to run (atmos,land,ocean)." &
                      //"  All tests are run by default.", &
                  requires_val=.true., &
                  long_name="--test")
call add_argument(parser, &
                  "-x", &
                  'Number of points (per domain tile) in the "x"' &
                      //" direction.  Defaults to 96.", &
                  requires_val=.true.)
call add_argument(parser, &
                  "-y", &
                  'Number of points (per domain tile) in the "y"' &
                      //" direction.  Defaults to 96.", &
                  requires_val=.true.)
call add_argument(parser, &
                  "-z", &
...
call add_argument(parser, &
                  "-d", &
                  "Have each rank occupy its own I/O domain" &
                      //" tile.  Defaults to one I/O domain" &
                      //" tile per domain tile.", &
                  requires_val=.false.)
call add_argument(parser, &
                  "-v", &
                  "Increase verbosity.", &
...
if (trim(buf) .ne. "not present") then
  ...
  elseif (trim(buf) .eq. "ocean") then
    ...
    write(error_unit) "unsupported value for -t." &
                      //"  Please use -h."
    stop 1
  endif
endif
...
if (tests(atmos)) then
  if (mod(npes,ntiles) .ne. 0) then
    call mpp_error(FATAL, &
                   "the number of ranks must be divisible by ntiles.")
  endif
...
if (tests(land)) then
  ...
  call create_land_domain(atmosphere_domain, &
                          nx, &
                          ny, &
                          ntiles, &
                          land_domain)
...
if (tests(ocean)) then
  call create_ocean_domain(ntiles*nx,ntiles*ny,npes,ocean_domain,ocn_layout,ocn_io_layout)
...
subroutine chksum_match(out_chksum, &
                        in_chksum, &
                        var_name, &
                        debug)
  ...
  if (debug) then
    write(error_unit,'(a,1x,i3.3,1x,a,1x,i20,1x,a,1x,i20,1x,a)') &
      trim(res)//"Rank",mpp_pe(), &
      "- chksum("//trim(var_name)//"):", &
      out_chksum,"(written)", &
      in_chksum,"(read) "//trim(color_end)
    call mpi_barrier(mpi_comm_world,err)
    call mpi_check(err)
  endif
  if (in_chksum .ne. out_chksum) then
    call mpp_error(fatal, &
                   "data that is read in does not match the" &
