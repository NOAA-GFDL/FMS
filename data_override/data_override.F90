...
!===============================================================================================
 ...
 use       fms2_io_mod, only: variable_exists, read_data, open_file, close_file, FmsNetcdfFile_t
 ...
 use data_override_mod, only: data_override_init, data_override, data_override_UG
  ...
  use mpp_domains_mod, only : mpp_get_ug_global_domain, mpp_global_field_ug, mpp_define_io_domain
 ...
 if(.not. open_file(fileobj, grid_file, 'read' )) then
     call mpp_error(FATAL, 'test_data_override: Error in opening file '//trim(grid_file))
 endif

 if(variable_exists(fileobj, "x_T" ) ) then
    call get_dimension_size(fileobj, "grid_x_T", nlon)
    call get_dimension_size(fileobj, "grid_y_T", nlat)
 else if(variable_exists(fileobj, "geolon_t" ) ) then
    call get_dimension_size(fileobj, "gridlon_t", nlon)
    call get_dimension_size(fileobj, "gridlat_t", nlat)
 else if (variable_exists(fileobj, "ocn_mosaic_file" )) then
    call read_data(fileobj, 'ocn_mosaic_file', solo_mosaic_file) 
    call close_file(fileobj)
    solo_mosaic_file = 'INPUT/'//trim(solo_mosaic_file)
    if(.not. open_file(fileobj, solo_mosaic_file, 'read' )) then
       call mpp_error(FATAL, 'test_data_override: Error in opening solo mosaic file '//trim(solo_mosaic_file))
    endif
    call get_dimension_size(fileobj, "ntiles", ntiles)
    if( ntiles .NE. 1) call error_mesg('test_data_override', 'only support single tile ocean mosaic, contact developer', FATAL)
    call read_data(fileobj, 'gridfiles', tile_file)
    tile_file = 'INPUT/'//trim(tile_file)
    call close_file(fileobj)
    if(.not. open_file(fileobj, tile_file, 'read' )) then
       call mpp_error(FATAL, 'test_data_override: Error in opening tile file '//trim(tile_file))
    endif
    call get_dimension_size(fileobj, "nx", siz(1))
    call get_dimension_size(fileobj, "ny", siz(2))
...
!=================================================================================================================================
 subroutine get_grid
   ...
   if(variable_exists(fileobj, 'x_T')) then
      ...
      call read_data(fileobj, 'x_vert_T', lon_vert_glo)
      call read_data(fileobj, 'y_vert_T', lat_vert_glo)
      ...
      allocate(lon_global  (nlon,  nlat    ), lat_global  (nlon,  nlat    ))
      call read_data(fileobj, 'geolon_vert_t', lon_vert_glo)
      call read_data(fileobj, 'geolat_vert_t', lat_vert_glo)
      ...
      do i = 1, nlon
         ...
         enddo
      enddo
   else ! reading from mosaic file
      allocate(lon_vert_glo(2*nlon+1,2*nlat+1,1), lat_vert_glo(2*nlon+1,2*nlat+1,1))
      allocate(lon_global  (nlon,  nlat    ), lat_global  (nlon,  nlat    ))
      call read_data( fileobj, 'x', lon_vert_glo)
      call read_data( fileobj, 'y', lat_vert_glo)  
      do j = 1, nlat
         do i = 1, nlon
